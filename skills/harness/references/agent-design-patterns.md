...
2. [에이전트 팀 아키텍처 유형](#아키텍처-유형) — 파이프라인, 팬아웃/팬인, 전문가 풀, 생성-검증, 감독자, 계층적 위임
3. [복합 패턴](#복합-패턴) — 패턴 조합 + **Gatekeeper-Advisor 메타 패턴**
4. [에이전트 타입 선택](#에이전트-타입-선택)
5. [에이전트 정의 구조](#에이전트-정의-구조) — 필수 섹션 + 팀 통신 프로토콜 템플릿
6. [에이전트 분리 기준](#에이전트-분리-기준) — 전문성·병렬성·컨텍스트·재사용성 4축
7. [스킬 vs 에이전트 구분](#스킬-vs-에이전트-구분)
8. [스킬 ↔ 에이전트 연결 방식](#스킬-에이전트-연결)
...
| **모든 패턴 + Gatekeeper-Advisor** | 기존 패턴 위에 교착 감지·Advisor 개입 층 추가 | 장기 실행 작업 — 에이전트가 루프에 빠지면 Advisor가 자동으로 접근법 재설정 |
...
### Gatekeeper-Advisor — 교착 대응 메타 패턴

Gatekeeper-Advisor는 특정 아키텍처 패턴이 아니라 **어느 패턴에도 덧붙일 수 있는 횡단 관심사(cross-cutting concern)**다. 에이전트가 루프에 빠지거나 상태 변화 없이 반복하는 경우 자동으로 고성능 Advisor를 호출하여 접근법을 재설정한다.

**추가 시점:** 다음 중 하나라도 해당하면 오케스트레이터에 통합한다.
- 작업이 외부 API·데이터에 의존하여 실패 가능성이 있을 때
- 에이전트가 스스로 교착을 인식하기 어려운 탐색적 작업일 때
- 작업 시간이 길고 중간 감시가 필요할 때

**두 감지 경로:**

| 경로 | 주체 | 감지 방식 |
|------|------|----------|
| Self-Correction | 작업 에이전트 내부 | 이터레이션마다 [P][C][D][B][R] 5축 자가 진단 → 임계값 충족 시 Advisor 호출 |
| Outer Loop | 오케스트레이터 | StateSnapshot 비교 → 연속 2회 무변화 시 Advisor 주입 |

**Advisor 호출 효율화:** Advisor(Opus)에게는 전체 히스토리 대신 ACP(Advisor Context Package, ≤680 토큰)로 압축하여 전달한다. 상세 구현: `skills/gatekeeper-advisor` 스킬 참조.
