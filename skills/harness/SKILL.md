...
**교착·반복 루프가 예상되는 작업:**
단순 1회 재시도로 해결되지 않는 교착(에이전트가 같은 접근을 반복하거나 진전 없이 멈추는 상황)에는 **Gatekeeper-Advisor 패턴**을 오케스트레이터에 통합한다.

- Self-Correction: 에이전트가 자가 진단 후 Advisor를 자발적으로 호출
- Outer Loop: 오케스트레이터가 상태 변화 없음을 감지 후 Advisor 주입
- Advisor 호출 시 ACP(≤680 토큰)로 압축하여 Opus 비용 최소화

> 에러 유형별 전략표와 구현 상세는 `references/orchestrator-template.md`의 "에러 핸들링" 참조.
> 교착 대응 패턴 전체: `skills/gatekeeper-advisor` 스킬 참조.
